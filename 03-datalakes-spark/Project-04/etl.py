...
def process_log_data(spark, input_data, output_data):
    ...
    df = df.where(col("page") == "NextSong").cache()

    # extract columns for users table    
    artists_table = df.select(col("firstName"), col("lastName"), col("gender"), col("level") \
                              ,col("userId")).distinct()
    
    # write users table to parquet files
    artists_table.write.parquet("{}users/users_table.parquet".format(output_data))
